...
export default function VolumeControl({ player, volumeLS } 
  : { player: any, volumeLS: string | null }) {

  const [prevVolume, setPrevVolume] = useState<string | null>(volumeLS)
  ...
  return (
    <div className={styles.volumeControl}>
      <button className={styles.volumeBtn} onClick={() => handleMute()}>
      ...
      <span ref={setVolumeTrack} className={styles.timeline}>
        <span className={styles.timelineThumb} style={{'--progress': `-${100 - (percent * 100)}%`}}>
        </span>
      </span>
    </div>
  )
